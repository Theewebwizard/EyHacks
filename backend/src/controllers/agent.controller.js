...
import { sendAgentForgotPasswordEmail } from "../lib/email.js";
...
export const forgotPassword = async (req, res) => {
    ...
    try {
        ...
        const agentInstance = await Agent.findOne({ agentID: new RegExp('^' + agentID.trim() + '$', 'i') });
        if (!agentInstance) {
            return res.status(404).json({ message: "No agent account found with this ID" });
        }
        ...
        agentInstance.password = hashedPassword;
        await agentInstance.save();

        // If agentID is formatted as an email, send email reset
        if (agentID.includes("@")) {
            await sendAgentForgotPasswordEmail(agentID.trim(), tempPassword);
        } else {
            logger.warn(`Agent ID ${agentID} is not an email. DEV RESET password: ${tempPassword}`);
        }

        res.status(200).json({ message: "Temporary password sent successfully" });
    } catch (error) {
        logger.error("Error in agent forgotPassword controller", { error: error.message });
        res.status(500).json({ message: "Internal server error" });
    }
};